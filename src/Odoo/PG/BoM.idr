...
toProduct_map [] = empty
toProduct_map (p@(MkRecordModel pk product_tmpl_id trade retail contract default_code) :: xs) = insert (ProdKey.pk32DX pk) p (toProduct_map xs)
--toProduct_map [] = empty
export
rbom_to_list : Maybe (List BrowseBoM.RecordModel) -> Hom1 --List (ProdKey,EQty)
rbom_to_list Nothing = []
rbom_to_list (Just x) = [ (ProdKey.pk32DX $product_id u,product_qty u) | u<-x]
export    
